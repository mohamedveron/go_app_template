...
Append-only log of human corrections to agent behaviour. Each entry records what the agent did, what the human corrected it to, and the suspected root cause. Patterns in this log feed the agent-harness-improvement review cycle.

**Do not delete or edit existing entries.** To close an entry, update its `status` field only.

See `agent-harness/skills/quality/log-agent-correction.md` for the how-to guide on writing entries.
