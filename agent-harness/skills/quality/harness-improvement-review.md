...
Open `harness/quality/agent-corrections.md` and read all entries with `status: open`.
...
Create a new exec-plan at `harness/plans/NNN-<jira-key>-harness-improvements/plan.md` following the standard exec-plan format. The plan should list each proposed change as a todo item, reference the correction entries that motivated it, and classify each change as:
