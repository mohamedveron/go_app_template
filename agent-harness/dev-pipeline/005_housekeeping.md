...
- `agent-harness/plans/NNN-<key>-short-desc/plan.md` — for context on what was built and why.
...
Create a `housekeeping_audit.md` file under the corresponding `agent-harness/plans/NNN-<key>-short-desc/` directory for the merged branch. This file should contain all the backlog entries prepared in Step 2, formatted as a readable audit log.
...
```bash
git mv agent-harness/plans/NNN-<key>-short-desc \
       agent-harness/plans/_archive/NNN-<key>-short-desc
```
...
If the work done in this PR touched any entry in `agent-harness/quality/debt.md`:
...
Do not add new debt entries here — new findings go into a new GitHub issue or the housekeeping audit from Step 3.

### Step 6 — Promote Correction Patterns

Read `agent-harness/quality/agent-corrections.md` and look for `status: open` entries related to the work in this PR.

- If any open entry describes a pattern you have now seen again in this PR's review cycle, that is a signal worth noting. Invoke `agent-harness/skills/quality/agent-harness-improvement-review.md` to assess whether a agent-harness change is warranted.
- If no open entries are relevant to this PR, skip this step.

### Step 7 — Open a PR

1. Create a new branch (e.g. `chore/housekeeping-NNN-<key>-short-desc`).
...
3. Push the branch and open a PR to `main` with:
   - A title like "Housekeeping — NNN-<key>-short-desc".
