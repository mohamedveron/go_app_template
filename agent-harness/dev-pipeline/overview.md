...
The agent-harness moves a task from plan through to merged code via a sequence of agent-driven stages with human checkpoints. Each stage is defined by a skill that the agent adopts and executes. Most stages run locally; only review and housekeeping run in the cloud.
...
- **Artifacts travel on the branch.** The coding plan, conversation summaries, and (post-merge) housekeeping audit all live under `agent-harness/plans/NNN-<key>-short-desc/` and are committed to the branch so that downstream agents have full context. See `agent-harness/plans/README.md` for the full naming convention.
