...
#   agent-harness/enforcement/utils/list-harness.sh knowledge skills      # only the named areas
#
...
for area in "${areas[@]}"; do
  ...
  target="$repo_root/agent-harness/$area"
  if [[ ! -d "$target" ]]; then
    echo "No such harness area: $area" >&2
    continue
  fi
  ( cd "$repo_root" && find "agent-harness/$area" -maxdepth 6 -print ) \
    | sort \
    | sed -e 's/[^\/]*\//│   /g' -e 's/│   \([^│]\)/├── \1/'
done
